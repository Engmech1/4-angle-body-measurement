...
class BodyMeasurementSystem:
    ...
    def compute_measurement(
        ...
        prior = self.SITE_MORPHOLOGY_PRIORS.get(site, {"lordosis_ratio": 0.120, "superellipse_p": 2.45})
        d_mean = (d_90 + d_270) / 2.0
        lordosis_val = custom_lordosis_cm if custom_lordosis_cm is not None else (d_mean * prior["lordosis_ratio"])
        p_val = custom_p if custom_p is not None else prior.get("superellipse_p", 2.45)
